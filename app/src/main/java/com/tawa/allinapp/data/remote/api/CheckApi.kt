...
interface CheckApi {
    ...
    fun sendCheckIn(@Header("Authorization") token: String,@Body request:CheckRemote.Check): Call<BaseResponse<CheckRemote.Response>>

    @POST("/api/asistencia/salida/add")
    fun sendCheckOut(@Header("Authorization") token: String,@Body request:CheckRemote.Check): Call<BaseResponse<CheckRemote.Response>>
